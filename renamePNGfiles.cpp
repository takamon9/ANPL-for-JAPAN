...
int main(void) {
	...
	string posFileName[2500];
	Mat posSrcImage[2500];
	Mat positiveImage[2500];
	string width, height;

	//string fnum = "numsets"; //Change this num "num_x"
	string fnum = "num_9"; //Change this num "num_x"
	int fileNumber = 1288; //Input File Number

	ofstream ofs("D:/numSets/" + fnum + "/pos/info.dat");
	int a = 1;
	...
	for (int i = 1; i < fileNumber + 1; i++) {  // input number of image files + 1
		...
		ssname << "D:/numSets/" + fnum + "/pos/num (" << a << ").png";

		string name = ssname.str();
		const char *cname = name.c_str();
		cout << cname << endl;

		ssrenamed << "D:/numSets/" + fnum + "/pos/" << a << ".png";
		...
		posSrcImage[i] = imread("D:/numSets/" + fnum + "/pos/" + posFileName[i]);
